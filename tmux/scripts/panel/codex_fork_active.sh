...
inner_script='command codex --ask-for-approval never --sandbox danger-full-access fork '"$session_id"'; code=$?; if [[ $code -ne 0 && $code -ne 130 && $code -ne 143 ]]; then echo; echo "codex fork 失败 (exit $code)"; read -r -n 1 -s -p "按任意键关闭..." || true; echo; fi; exec "${SHELL:-/bin/zsh}" -l'
